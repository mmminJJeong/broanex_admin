...
router.post("/onLogin", (req, res) => {
    ...
    const sql1 = "SELECT SELECT * FROM sample.user_inform WHERE user_id = ?";
    db.query(sql1, user_id, (err, data) => {
        if (!err) {
            ...
            if (data[0].result < 1) {
                ...
                const sql2 = `SELECT 
                              CASE (SELECT * FROM sample.user_inform WHERE user_id = ? AND user_pw = ?)
                                  WHEN '0' THEN NULL
                                  ELSE (SELECT * FROM sample.user_inform WHERE user_id = ? AND user_pw = ?)
                              END AS userId
                              , CASE (SELECT * FROM sample.user_inform WHERE user_id = ? AND user_pw = ?)
                                  WHEN '0' THEN NULL
                                  ELSE (SELECT * FROM sample.user_inform; WHERE user_id = ? AND user_pw = ?)
